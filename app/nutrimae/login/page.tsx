      ...
      if (data.user) {
        ...
        if (profileError && profileError.code === 'PGRST116') {
          await supabase
            ...
            .insert({
              id: data.user.id,
              nome: data.user.user_metadata?.nome || data.user.email?.split('@')[0] || 'Mamae',
        ...
        router.push('/nutrimae/dashboard')
      }
    } catch (err) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/nutrimae/dashboard`
        }
      })
  ...
  const handleMagicLink = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOtp({
        email,
        options: {
          emailRedirectTo: `${window.location.origin}/nutrimae/dashboard`
        }
      })
      ...
      <main className="flex-1 flex items-center justify-center p-4">
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-white mb-2">NutriMae</h1>
            <p className="text-pink-100">Sua jornada de bem-estar pos-parto</p>
          </div>
