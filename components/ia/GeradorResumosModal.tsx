...
import { useResumosIA, ConfigResumo } from '@/hooks/useResumosIA'
...
export function GeradorResumosModal({ isOpen, onClose, onSuccess }: Props) {
  const { gerarResumo, gerando } = useResumosIA()
  ...
  const [step, setStep] = useState<'form' | 'gerando' | 'sucesso'>('form')
  ...
  const handleGerar = async () => {
    ...
    const result = await gerarResumo({
      texto,
          ...
          {step === 'gerando' && (
            <div className="flex flex-col items-center justify-center py-12 gap-4">
              <div className="size-16 rounded-full bg-purple-500/20 flex items-center justify-center">
                <span className="material-symbols-outlined text-4xl text-purple-500 animate-spin">progress_activity</span>
              </div>
              <h3 className="text-lg font-bold text-gray-900 dark:text-white">Gerando resumo...</h3>
              <p className="text-sm text-[#9dabb9] text-center max-w-sm">
                A IA está processando o texto e criando um resumo organizado.
              </p>
            </div>
          )}

          {step === 'sucesso' && resumoGerado && (
            <div className="flex flex-col gap-4">
              <div className="flex items-center gap-2 p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                <span className="material-symbols-outlined text-emerald-500">check_circle</span>
                <p className="text-sm text-emerald-500">Resumo gerado com sucesso!</p>
              </div>

              {/* Resumo gerado */}
              <div className="p-4 rounded-lg bg-gray-50 dark:bg-[#141A21] border border-gray-200 dark:border-[#283039] max-h-[400px] overflow-y-auto">
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={handleReset}
                  className="flex-1 py-2.5 bg-purple-500 hover:bg-purple-600 text-white font-bold rounded-lg transition-colors flex items-center justify-center gap-2"
                >
