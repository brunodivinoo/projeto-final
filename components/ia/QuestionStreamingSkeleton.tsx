...
export default function QuestionStreamingSkeleton({
  ...
  return (
    <div className="bg-[#1A2332] border border-white/10 rounded-xl overflow-hidden shadow-lg animate-pulse-subtle">
      {/* Header Skeleton */}
      <div className="flex flex-wrap items-center gap-2 px-4 py-3 bg-[#151D2B] border-b border-white/10">
        ...
        {partialData?.disciplina ? (
          <div className="flex items-center gap-1.5 text-sm">
            <span className="text-white/90 font-medium">{partialData.disciplina}</span>
            {partialData.assunto && (
              <>
                <span className="text-white/40">›</span>
                <span className="text-white/60">{partialData.assunto}</span>
              </>
            )}
          </div>
        ) : (
      ...
      <div className="p-4 lg:p-5">
        ...
        {partialData?.caso_clinico ? (
          <div className="mb-4 p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
            ...
            <p className="text-white/80 text-sm leading-relaxed whitespace-pre-wrap">
              {partialData.caso_clinico}
              <span className="inline-block w-2 h-4 bg-emerald-400/50 animate-blink ml-1" />
            </p>
          </div>
        ) : elapsedTime > 2000 && (
          <div className="mb-4 p-3 bg-blue-500/5 border border-blue-500/10 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              ...
              <div className="h-3 w-24 bg-blue-400/20 rounded animate-pulse" />
            </div>
        ...
        <div className="space-y-2">
          {['A', 'B', 'C', 'D', 'E'].map((letra, idx) => (
            ...
            >
              <span className="flex-shrink-0 w-7 h-7 rounded-full bg-white/5 flex items-center justify-center text-white/30 text-sm font-medium">
                {letra}
              </span>
        ...
        <div className="mt-6 pt-4 border-t border-white/10">
          <div className="flex items-center justify-center gap-3">
            ...
            <div className="flex items-center gap-2 text-white/70">
              <span className="text-lg">{currentProgress.emoji}</span>
              <span className="text-sm font-medium">{currentProgress.text}</span>
            </div>
          </div>

          {/* Barra de progresso visual (não linear, apenas visual) */}
              ...
              {onCancel && (
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 text-xs font-medium bg-white/5 hover:bg-white/10 text-white/70 rounded-lg transition-colors"
                  >
                    Aguardar mais
                  </button>
                  <button
