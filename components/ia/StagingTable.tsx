        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 grid grid-cols-3 gap-4">
            <div className="text-center">
              <span className="text-xs text-slate-500 uppercase">T</span>
              <p className="text-white font-medium">{row.t}</p>
            </div>
            <div className="text-center">
              <span className="text-xs text-slate-500 uppercase">N</span>
          ...
          <div className="w-40">
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs text-slate-500">Sobrevida 5 anos</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                {row.treatment && (
                  <div className="bg-slate-900/50 rounded-lg p-3">
                    <span className="text-xs text-slate-500 uppercase block mb-2">Tratamento</span>
                ...
                {row.notes && (
                  <div className="bg-slate-900/50 rounded-lg p-3">
                    <span className="text-xs text-slate-500 uppercase block mb-2">Observações</span>
                    <p className="text-slate-700 text-sm">{row.notes}</p>
                  </div>
                )}
                ...
                {row.prognosticFactors && row.prognosticFactors.length > 0 && (
                  <div className="bg-slate-900/50 rounded-lg p-3">
                    <span className="text-xs text-slate-500 uppercase block mb-2">Fatores Prognósticos</span>
                ...
                {row.clinicalNotes && !row.prognosticFactors?.length && (
                  <div className="bg-slate-900/50 rounded-lg p-3">
                    <span className="text-xs text-slate-500 uppercase block mb-2">Notas Clínicas</span>
    ...
    `}>
      ...
      <div className="p-4 border-b border-slate-200 bg-slate-100">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {cancerType && (
                <p className="text-slate-600 text-sm">{cancerType}</p>
              )}
            </div>
          </div>
            ...
            <div className="flex items-center gap-1 mr-2">
              ...
              <button
                ...
                className={`px-2 py-1 text-xs rounded transition-colors ${
                  filter === null ? 'bg-purple-500/30 text-purple-300' : 'text-slate-500 hover:text-slate-600'
                }`}
              >
                Todos
              </button>
              {uniqueStages.sort().map(stage => (
                <button
                  ...
                  className={`px-2 py-1 text-xs rounded transition-colors ${
                    filter === stage ? 'bg-purple-500/30 text-purple-300' : 'text-slate-500 hover:text-slate-600'
                  }`}
                >
                  {stage}
                </button>
        ...
        <div className="flex items-center gap-4 mt-4 pt-4 border-t border-slate-200">
          <span className="text-xs text-slate-500">Prognóstico:</span>
  ...
  if (isFullscreen) {
    return (
      <>
        ...
        <div className="fixed inset-4 z-50 bg-slate-900 rounded-2xl border border-slate-200 overflow-hidden flex flex-col shadow-2xl">
          {content}
        </div>
      </>
    )
